...
function listTrashCanFiles(){
    ...
    if [[ $(ls -A ~/.trashCan/) ]]; then
        ...
        ls -l $TRASHCAN_PATH | awk 'NF>2 { 
                                        ...
                                        printf  "%15s %10s\n", $9, $5
                                }' | more  #it redirects the ls result to awk through pipe and awk prints the columns 9, 5 and 1 from ls.
    else
        echo ""
        echo ""
...
Purpose: Creates a subprocess to monitor the changes in the trashCan folder

------------------------------------------------------------------'
function monitor(){
    # gnome-terminal -- "bash ./monitor.sh"
    x-terminal-emulator -e "bash ./monitor.sh" #calling the a terminal to execute the monitor script
}

:'-------------------------------------------------------------------

Function Name: kill_monitor

Purpose: Ending with the monitor subprocess

------------------------------------------------------------------'
function kill_monitor(){
    monitor_id=$(ps -eaf | pgrep -f monitor.sh)  #finds the PID of the monitor process
    kill -9 $monitor_id #kill the monitor process
}
...
Purpose: Recovers a file from the trashCan to the current working directory

------------------------------------------------------------------'
function recoverFile(){
    if [[ -f "$TRASHCAN_PATH"/"$1" ]] || [[ -d "$TRASHCAN_PATH"/"$1" ]] #check if file or folder to recovered exist
    then
        result=$(mv "$TRASHCAN_PATH"/"$1" .) #moves the files to the current folder
        echo "$(tput bold)$(tput setaf 3) File $1 recovered successfully and moved to $(pwd) $(tput sgr 0)" #displays a message
    else
        echo "$(tput bold)$(tput setaf 1)Error recovering the file. Ensure the file $1 exists and it is the file to be recovered $(tput sgr 0)" #informs the user the file was not found
        printf "\n\n"
...
function folderUsage(){
    ...
    if [[ $(ls -A ~/.trashCan/) ]]; then #checking if folder is empty
        for file in ~/.trashCan/*
        do 
            ((total+=$( ls -l $file | awk '{print $5}' ))) 
        done
